...
void Model::MakeTriangle(Vertex v0, Vertex v1, Vertex v2) {
	std::vector<ModelVertex> vtx = {
		{v0.GetPos(), v0.GetNorm()},
		{v1.GetPos(), v1.GetNorm()},
		{v2.GetPos(), v2.GetNorm()},
	};
...
void Model::MakeSkin(std::vector<Vertex> vertices, std::vector<Triangle> indices) {
	// Specify vertices
	std::vector<ModelVertex> vtx;
	
	for (auto& v : vertices) {
		vtx.push_back({ v.GetPos(), v.GetNorm() });
	}
